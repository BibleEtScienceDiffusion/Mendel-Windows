...
function validate(obj) {
  ...
  var warning = parseFloat(obj.dataset.warning)

  if (val < min || val > max) {
    obj.parentNode.parentNode.className = "form-group has-error";
  } else if (val > warning && val <= max) {
    obj.parentNode.parentNode.className = "form-group has-warning";
  } else {
...
function compute_memory() {
  ...
  if (mem_est > 500) {
     document.getElementById("danger").innerText = "DANGER: you are using more than 500MB of memory! Thrashing may occur!";
  } else {
